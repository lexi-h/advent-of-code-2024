    ...
    for (const auto& [key,v] : d){
        for(int i = 0; i < v.size(); ++i){
            for(int j = i+1; j < v.size(); ++j){
                ...
                while(is_pair_inbounds(g, antinode0)){
                    g[antinode0.first][antinode0.second].is_antinode = true;
                    antinode0.first -= xdiff;
                    antinode0.second -= ydiff;
                }
                while(is_pair_inbounds(g, antinode1)){
                    g[antinode1.first][antinode1.second].is_antinode = true;
                    antinode1.first += xdiff;
                    antinode1.second += ydiff;
                }
                //add antinodes at the antennae too
                g[a.first][a.second].is_antinode = true;
                g[b.first][b.second].is_antinode = true;
            }
        }
    }
}
